...
- Choose the minimum sufficient execution surface: use a notebook for conceptual, numerical, or visualization-heavy inquiry; use headless MuJoCo plus a structured artifact for embodied questions; promote stabilized reusable behavior into source code; preserve behavioral claims in reproducible experiments. MuJoCo measures and Jupyter interrogates—neither replaces the other.
- Write headless experiment artifacts to `results/` using the shared JSON telemetry schema in `experiments/artifacts.py`. Record schema version, experiment source, model/timestep provenance, run parameters, termination state, timestamped samples, and summary metrics. Keep run outputs ignored; notebooks must be able to load them without rerunning the simulator.
- Keep notebooks in `notebooks/` as reviewable analyses, derivations, or learning artifacts. Notebook state is not authoritative: do not leave reusable implementation or physical claims notebook-only. Include visible input paths and explicit validation/reproduction commands in notebook-facing documentation.
