...
- Treat the post-foundation destination as an open-ended deep-toy platform for procedural validated worlds and reproducible rollout populations. Do not pull this frontier ahead of a current prerequisite such as standing or locomotion.
- When an agentic control plane becomes useful, compile natural-language goals into typed experiment specifications and typed operations. Deterministic validators and simulator contracts are authoritative over LLM reasoning.
- Enforce scientific policy in data structures and execution boundaries: keep hypotheses, interventions, controls, and measurements distinct; retain invalid worlds and failures; record code, simulator, seed, distribution, and metric provenance; make post-hoc protocol changes new records; and never allow hidden-parameter changes to rescue an experiment silently.
